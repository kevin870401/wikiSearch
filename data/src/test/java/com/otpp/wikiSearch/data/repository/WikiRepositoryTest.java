...
public class WikiRepositoryTest extends AbstractTestNGSpringContextTests {
    ...
    public void findByTitleIn_plainString_exist()  {
        ...
        Page<WikiArticle> wikiArticles=wikiRepository.findByTitleIn(QueryUtil.splitSearchTermAndRemoveIgnoredCharacters(AFGHANISTAN), pageable);
        assertThat(wikiArticles.getTotalPages()==1);
        assertThat(wikiArticles.getContent().get(0).getTitle()).isEqualTo(AFGHANISTAN);
    }

    @Test
    public void findByTitleIn_StringContainsSpace_notExist()  {
        Pageable pageable= new PageRequest(0,10);
        Page<WikiArticle> wikiArticles=wikiRepository.findByTitleIn(QueryUtil.splitSearchTermAndRemoveIgnoredCharacters("Afghanistan communication"), pageable);
        assertThat(wikiArticles.getTotalPages()==1);
        assertThat(wikiArticles.getContent().size()==0);
    }
    ...
    public void findByTitleStartsWith_afghanistan_9ArticlesFound(){
        Pageable pageable= new PageRequest(0,10);
        Page<WikiArticle> wikiArticles=wikiRepository.findByTitleStartsWith(QueryUtil.splitSearchTermAndRemoveIgnoredCharacters(AFGHANISTAN), pageable);
        assertThat(wikiArticles.getSize()).isEqualTo(10);
        assertThat(wikiArticles.getTotalPages()).isEqualTo(1);
        assertThat(wikiArticles.getContent().size()).isEqualTo(9);
    }

    @Test
    public void findByTitleStartsWith_a_highestUserCountFoundInFacet(){
        Pageable pageable= new PageRequest(0,10);
        FacetPage<WikiArticle> wikiArticles=wikiRepository.findByTitleStartsWith(QueryUtil.splitSearchTermAndRemoveIgnoredCharacters("A"), pageable);
        assertThat(((SimpleFacetFieldEntry)((SolrResultPage)wikiArticles.getFacetResultPages().iterator().next()).getContent().iterator().next()).getValueCount()).isEqualTo(47);
        assertThat(((SimpleFacetFieldEntry)((SolrResultPage)wikiArticles.getFacetResultPages().iterator().next()).getContent().iterator().next()).getValue()).isEqualTo("Jim Carter");
    }
    ...
    public void countByTitleStartsWith_afghanistan_9ArticlesFound(){
        assertThat(wikiRepository.countByTitleStartsWith(QueryUtil.splitSearchTermAndRemoveIgnoredCharacters(AFGHANISTAN))).isEqualTo(9);
